...
function initializeAuth() {
    ...
    // Actualizar enlaces del dashboard según el rol
...
function updateAuthUI(isAuthenticated) {
    ...
    if (isAuthenticated) {
        ...
        // Obtener el rol del usuario y actualizar enlaces
        getUserRoleAndUpdateLinks();
...
/**
 * Obtener el rol del usuario y actualizar los enlaces del dashboard
 */
function getUserRoleAndUpdateLinks() {
    // Obtener el rol desde el elemento HTML renderizado por Thymeleaf
    const userRoleElement = document.querySelector('.user-role, .welcome-role');
    
    if (userRoleElement) {
        const rolText = userRoleElement.textContent.trim().toUpperCase();
        
        // Determinar la URL del dashboard según el rol
        let dashboardUrl = '/dashboard';
        
        if (rolText.includes('ADMINISTRADOR') || rolText === 'ADMINISTRADOR') {
            dashboardUrl = '/dashboard/admin';
            AppState.userRole = 'ADMINISTRADOR';
        } else if (rolText.includes('EMPLEADO') || rolText === 'EMPLEADO') {
            dashboardUrl = '/dashboard/empleado';
            AppState.userRole = 'EMPLEADO';
        } else if (rolText.includes('CLIENTE') || rolText === 'CLIENTE') {
            dashboardUrl = '/dashboard/cliente';
            AppState.userRole = 'CLIENTE';
        }
        
        // Actualizar todos los enlaces del dashboard
        updateDashboardLinksWithUrl(dashboardUrl);
    }
}

/**
 * Actualizar todos los enlaces del dashboard con la URL correcta
 */
function updateDashboardLinksWithUrl(dashboardUrl) {
    // Actualizar enlace en el menú dropdown
    const dropdownDashboardLink = document.querySelector('.dropdown-menu a[href="/dashboard"]');
    if (dropdownDashboardLink) {
        dropdownDashboardLink.href = dashboardUrl;
    }
    
    // Actualizar enlace en el mensaje de bienvenida
    const welcomeLink = document.querySelector('.welcome-link');
    if (welcomeLink) {
        welcomeLink.href = dashboardUrl;
    }
    
    // Actualizar botón en la sección de servicios
    const infoBtnLink = document.querySelector('.info-btn');
    if (infoBtnLink) {
        infoBtnLink.href = dashboardUrl;
    }
    
    // Actualizar enlace en el footer
    const footerAuthLink = document.getElementById('footerAuthLink');
    if (footerAuthLink) {
        footerAuthLink.innerHTML = `<a href="${dashboardUrl}">Mi Dashboard</a>`;
    }
}

/**
 * Actualizar enlaces del dashboard al cargar la página
 */
function updateDashboardLinks() {
    // Esta función se llama al inicializar para configurar los enlaces correctos
